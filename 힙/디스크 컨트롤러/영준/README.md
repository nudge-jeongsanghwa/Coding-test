...
jobs의 길이를 n이라고 한다면, 처음에 정렬하는 부분의 시간복잡도는 O(nlogn), 반복문을 도는 부분은 O(n)이다. 반복문 내부의 경우 시간이동을 통해 결과적으로 작업의 갯수만큼 실행되므로 선형 복잡도를 갖는다. 따라서 시간복잡도는 O(nlogn)이다. n <= 500이므로 적절한 풀이이다.
